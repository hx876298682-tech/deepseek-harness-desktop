 ...
 * quota windows, a native-accent daily token-usage heatmap, per-day
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".usg_accountCard{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1,var(--dsw-alias-bg-base));border-radius:12px;padding:10px 11px;display:flex;flex-direction:column;gap:9px}",
			".usg_accountHead{align-items:center;gap:8px;display:flex}",
			".usg_accountMark{width:24px;height:24px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-fill-l2);border:1px solid var(--dsw-alias-border-l2);border-radius:7px;justify-content:center;align-items:center;font-size:10px;font-weight:600;display:flex}",
			...
			".usg_accountStatus[data-status=ok]{color:var(--dsw-alias-state-success-primary);background:var(--dsw-alias-state-success-tertiary)}",
			...
			".usg_quotaFill{height:100%;background:var(--usg-accent);border-radius:inherit;min-width:2px;transition:width .2s ease}.usg_quotaFill[data-tone=warning]{background:var(--dsw-alias-state-warn-primary)}.usg_quotaFill[data-tone=critical]{background:var(--dsw-alias-state-error-primary)}",
			...
			".usg_heatToggle{cursor:pointer;flex:none;align-items:center;gap:3px;border:0;background:0 0;border-radius:6px;padding:2px 4px;margin-left:-5px;font:inherit;color:var(--dsw-alias-label-tertiary);display:flex}",
			".usg_heatToggle:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}",
			...
			".usg_cellToday{box-shadow:0 0 0 1px var(--usg-accent)}",
			".usg_cellToday:hover{box-shadow:0 0 0 1px var(--usg-accent)}",
			...
			".usg_dayBar{background:var(--usg-accent);border-radius:2px;height:6px;flex:1;min-width:4px;opacity:.65}",
			...
			".usg_modelBar{background:var(--usg-accent);border-radius:2px;height:5px}",
		...
		/**
		 * Native accent cell color: continuous square-root mapping against the
		 * month's max (more tokens → strictly deeper DeepSeek accent, no
		 * banding), rendered as a plain rgba overlay of --dsw-static-deepseek-500
		 * so it works in every browser/theme without color-mix support. Zero is
		 * the neutral gray "empty" cell. Returns the cell's background and text color.
		 */
		const BLUE_RGB = [65, 118, 230];
		...
		function UsageStatsPanel({ wide, t }) {
			...
			// 面板每次打开时默认折叠月历热力图，避免遮挡视线
			react.useEffect(() => {
				if (open) setHeatOpen(false);
			}, [open]);
												...
												children: [
													react_jsx_runtime.jsxs("div", {
														...
														children: [
															react_jsx_runtime.jsx("button", {
																type: "button",
																className: S.heatToggle,
																"aria-expanded": heatOpen,
																"aria-controls": "dsh-usage-stats-heatmap",
																onClick: () => setHeatOpen((value) => !value),
																children: [
																	heatOpen
																		? react_jsx_runtime.jsx(primitives.IconChevronDownOutline14, { size: 12 })
																		: react_jsx_runtime.jsx(primitives.IconChevronRightOutline14, { size: 12 }),
																	react_jsx_runtime.jsx("span", { className: S.sectionTitle, children: translate("usage.heatmap") })
																]
															}),
															heatOpen && react_jsx_runtime.jsxs("div", {
															...
															heatOpen && react_jsx_runtime.jsx(MonthHeatmap, {
														heat,
														translate,
		...
		function SubscriptionContent({ provider, translate }) {
			...
			return (status === "ok" || provider.stale === true) && windows.length > 0 ? react_jsx_runtime.jsx("div", {
						...
						children: windows.map((window) => {
							...
							const tone = remaining === null ? null : remaining <= 10 ? "critical" : remaining <= 30 ? "warning" : null;
							return react_jsx_runtime.jsxs("div", {
								...
								children: [
									...
									react_jsx_runtime.jsx("div", {
										...
										"aria-valuemax": 100,
										"aria-valuenow": used,
										children: react_jsx_runtime.jsx("div", { className: S.quotaFill, "data-tone": tone, style: { width: `${used}%` } })
									})
								]
							}, window.kind);
						})
		...
		/**
		 * Native-accent calendar heatmap for one month: weekday header row,
