...
contextBridge.exposeInMainWorld("__DSH_DESKTOP_USAGE_STATS__", {
  status: () => ipcRenderer.invoke("dsh-usage-stats:status"),
  install: () => ipcRenderer.invoke("dsh-usage-stats:install")
});
...
const USAGE_STATS_WORLD = String.raw`(() => {
  if (window.__DSH_DESKTOP_USAGE_STATS_UI__) return;
  window.__DSH_DESKTOP_USAGE_STATS_UI__ = true;
  const api = window.__DSH_DESKTOP_USAGE_STATS__;
  const css = "[data-dsh-usage-panel]{color:var(--dsw-alias-label-primary);padding:4px 0;max-width:520px}[data-dsh-usage-state]{color:var(--dsw-alias-label-tertiary);font-size:14px;line-height:22px}[data-dsh-usage-ok]{color:var(--dsw-alias-state-success-primary);font-size:14px;line-height:22px}[data-dsh-usage-error]{color:var(--dsw-alias-state-error-primary);font-size:14px;line-height:22px}[data-dsh-usage-install]{border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-primary);border-radius:8px;padding:6px 12px;cursor:pointer;margin-top:10px}[data-dsh-usage-note]{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:20px;margin-top:8px}";
  const isPluginTabList = (list) => Array.from(list.querySelectorAll("[role=tab]")).some(tab => /插件配置|plugin configuration/i.test(tab.textContent || ""));
  const create = (tag, attrs = {}, text = "") => { const node = document.createElement(tag); for (const [key, value] of Object.entries(attrs)) node.setAttribute(key, value); if (text) node.textContent = text; return node; };
  const panelFor = (tablist) => tablist.parentElement?.querySelector("[role=tabpanel]") || tablist.parentElement?.lastElementChild;
  const render = async (panel) => {
    panel.replaceChildren();
    panel.appendChild(create("div", { "data-dsh-usage-state": "" }, "正在读取插件状态…"));
    const showInstall = (note) => {
      const button = create("button", { type: "button", "data-dsh-usage-install": "" }, "安装 / 更新");
      button.addEventListener("click", async () => {
        button.disabled = true; button.textContent = "正在安装…";
        try { await api.install(); await render(panel); }
        catch (error) { panel.replaceChildren(create("div", { "data-dsh-usage-error": "" }, "安装失败：" + (error?.message || String(error)))); }
      });
      panel.appendChild(button);
      if (note) panel.appendChild(create("div", { "data-dsh-usage-note": "" }, note));
    };
    try {
      const status = await api.status();
      panel.replaceChildren();
      if (status && status.installed && status.enabled) {
        panel.appendChild(create("div", { "data-dsh-usage-ok": "" }, "已安装 dsh-usage-stats v" + (status.version || "?") + "，已启用"));
        panel.appendChild(create("div", { "data-dsh-usage-note": "" }, "重启 dsh web 并硬刷新浏览器后，侧边栏底部会出现“用量/余额”入口。"));
        showInstall("再次点击可把插件更新为当前应用自带的版本。");
      } else {
        panel.appendChild(create("div", { "data-dsh-usage-state": "" }, "未安装 dsh-usage-stats 插件"));
        showInstall("安装后应用每次启动会自动保持同步。");
      }
    } catch (error) { panel.replaceChildren(create("div", { "data-dsh-usage-error": "" }, "读取插件状态失败：" + (error?.message || String(error)))); }
  };
  const install = () => {
    const tablist = Array.from(document.querySelectorAll("[role=tablist]")).find(isPluginTabList);
    if (!tablist || tablist.querySelector("[data-dsh-usage-tab]")) return;
    const existingTabs = Array.from(tablist.querySelectorAll("[role=tab]"));
    const usageTab = create("button", { type: "button", role: "tab", "data-dsh-usage-tab": "", "aria-selected": "false" }, "用量统计");
    const panel = create("div", { "data-dsh-usage-panel": "", role: "tabpanel", hidden: "" });
    const host = tablist.parentElement;
    if (!host) return;
    tablist.appendChild(usageTab); host.appendChild(panel);
    usageTab.addEventListener("click", () => { existingTabs.forEach(tab => tab.setAttribute("aria-selected", "false")); usageTab.setAttribute("aria-selected", "true"); const oldPanel = panelFor(tablist); if (oldPanel && oldPanel !== panel) oldPanel.hidden = true; panel.hidden = false; void render(panel); });
    existingTabs.forEach(tab => tab.addEventListener("click", () => { if (usageTab.getAttribute("aria-selected") === "true") usageTab.setAttribute("aria-selected", "false"); panel.hidden = true; }));
  };
  const start = () => {
    if (!document.documentElement) { setTimeout(start, 50); return; }
    const style = document.createElement("style"); style.textContent = css; document.head?.appendChild(style);
...
webFrame.executeJavaScript(USAGE_STATS_WORLD).catch(() => {});
